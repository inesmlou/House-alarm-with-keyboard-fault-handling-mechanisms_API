function lines= PN_device_kb_IO_C(act, t)
...
for t= 0:.1:10
    keys= zeros(1,12);
    [~,~,v]= find( PN_device_kb_IO_C([1 0 0], t).*col1 ); keys(v)=1;
    [~,~,v]= find( PN_device_kb_IO_C([0 1 0], t).*col2 ); keys(v)=1;
    [~,~,v]= find( PN_device_kb_IO_C([0 0 1], t).*col3 ); keys(v)=1;
    kSav= [kSav; keys];
end
